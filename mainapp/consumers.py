...
import json
from mainapp.models import StockDetails
...
class StockConsumer(AsyncWebsocketConsumer):
    ...
    def addToCeleryBeat(self, stockpicker):
        task = PeriodicTask.objects.filter(name = "every-10-secounds")
        if len(task)>0:
            task = task.first()
            ...
            task = PeriodicTask.objects.create(interval = schedule, name = 'every-10-secounds', task="mainapp.tasks.update_stock", args=json.dumps([stockpicker]))
    ...
    def helper_func(self):
        user = self.scope["user"]
        stocks = StockDetails.objects.filter(user_id = user.id)
        task = PeriodicTask.objects.get(name = "every-10-secounds")
        args = json.loads(task.args)
        args = args[0]
        for i in stocks:
            i.user.remove(user)
            if i.user.count() == 0:
                args.remove(i.stocks)
                i.delete()
    ...
    async def send_stock(self, text_data):
